...
def build_learning_graph(
    ...
    *,
    ...
    deterministic ``prepare_practice`` agent. The session starts by recalling
    long-term learner memory from the store and ends by persisting it; code
    submissions pause on an approval interrupt before execution. Model nodes
    retry transient provider errors; the pure ``make_diagnostic`` node
