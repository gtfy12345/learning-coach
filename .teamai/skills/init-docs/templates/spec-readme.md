# 设计文档规范

## 1 目录结构

```
docs/spec/
├── README.md                    # 本文件，规范说明
├── INDEX.md                     # 文档索引
└── ${subject}-${type}.md        # 设计文档
```

## 2 文档元信息

所有设计文档必须在头部包含元信息：

```markdown
> **版本**: 1.0
> **状态**: active
> **更新日期**: 2025-12-27
```

**版本号说明**：
- **X（主版本）**：文档结构或目标发生重大变更时递增
- **Y（次版本）**：内容修订、补充细节时递增

**状态值说明**：

| 状态 | 含义 |
|------|------|
| `draft` | 草稿，尚未正式生效 |
| `active` | 生效中，当前有效版本 |
| `superseded` | 已被取代，需注明新文档路径 |
| `deprecated` | 已废弃，不再适用 |

## 3 命名规范

| 类型 | 命名模式 | 示例 |
|------|----------|------|
| 架构设计 | `${subject}-architecture.md` | `system-architecture.md` |
| 模块设计 | `${module}-design.md` | `font-renderer-design.md` |
| 数据结构 | `${subject}-schema.md` | `config-schema.md` |
| 接口设计 | `${subject}-interface.md` | `plugin-interface.md` |

## 4 文档模板

### 4.1 模块设计模板

```markdown
# 模块名称 设计文档

> **版本**: 1.0
> **状态**: active
> **更新日期**: YYYY-MM-DD

## 1 概述

简要描述模块的目的和职责。

## 2 设计目标

- 目标 1
- 目标 2

## 3 架构设计

描述模块的整体架构，可包含图表。

## 4 接口定义

### 4.1 公开接口

描述对外暴露的接口。

### 4.2 内部接口

描述模块内部的关键接口。

## 5 数据结构

定义核心数据结构。

## 6 错误处理

描述错误处理策略。

## 7 验收标准

| ID | 场景 | Given | When | Then | Phase |
|----|------|-------|------|------|-------|
| C-1 | 场景描述 | 前置条件 | 触发动作 | 预期结果 | 对应 Phase |

## 8 关联文档

- [相关文档1](./path1.md)
- [相关文档2](./path2.md)
```

> 验收标准为可选 section。小型无取舍的文档可省略。
> `ID` 列是文档内的说明性编号，不作为 BDD 场景编号。
> 需要 BDD 闭环时，应在 `bdd-test-plan.md` 中分配场景编号，并遵守对应 `test/scenarios/<layer>/README.md` 与 `INDEX.md` 的编号规范；implementation checklist 的 `BDD-Gate` 仅引用这些场景编号。

## 5 检查清单

完成设计文档前，确认以下事项：

- [ ] 文件命名符合规范
- [ ] 包含元信息头（版本、状态、更新日期）
- [ ] 已更新 `INDEX.md` 索引
- [ ] 版本号已递增（如为更新）
- [ ] 状态值正确设置
